...
export class Horizon {
  ...
  enableAltGameMode(spritePos) {
    ...
    Obstacle.MAX_GAP_COEFFICIENT =
      window.Runner.spriteDefinition.MAX_GAP_COEFFICIENT;
    Obstacle.MAX_OBSTACLE_LENGTH =
      window.Runner.spriteDefinition.MAX_OBSTACLE_LENGTH;

    BackgroundEl.config = window.Runner.spriteDefinition.BACKGROUND_EL_CONFIG;
  ...
  addNewObstacle(currentSpeed) {
    const obstacleCount =
      ...
      (window.Runner.isAltGameModeEnabled() && !this.altGameModeActive) ||
    ...
    if (
      ...
      if (this.obstacleHistory.length > 1) {
        this.obstacleHistory.splice(
          window.Runner.config.MAX_OBSTACLE_DUPLICATION
        );
      }
    }
  }
  ...
  addBackgroundEl() {
    const backgroundElTypes = Object.keys(
      window.Runner.spriteDefinition.BACKGROUND_EL
    );
